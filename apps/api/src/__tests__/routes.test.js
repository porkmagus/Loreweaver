...
import { resolveImageProviderConfig } from '../services/imageProvider.js';
import { sendCharacterChat, getChatHistory, streamCharacterChat, buildCognitionContext, getLatestSessionForCharacter, listCharacterChatSessions, } from '../services/chatService.js';
...
vi.mock('../db/client.js', () => {
    return {
        db: {
            ...
            insert: () => ({ values: (vals) => ({ returning: mockReturning }) }),
        },
    };
});
...
vi.mock('../services/chatService.js', () => ({
    ...
    toCognitionSnapshot: vi.fn((cognition) => cognition),
}));
...
describe('GET /api/characters', () => {
    it('lists characters', async () => {
        const a = await readyApp();
        vi.mocked(listCharacters).mockResolvedValue([{ id: 1, name: 'Aragorn', worldId: 1 }]);
        const res = await request(a.server)
...
describe('POST /api/chat/character/:id', () => {
    ...
    it('returns 400 when character world mismatch', async () => {
        const a = await readyApp();
        vi.mocked(getCharacterById).mockResolvedValue({ id: 1, worldId: 2, name: 'Alice' });
        const res = await request(a.server)
            .post('/api/chat/character/1')
            .send({ worldId: 1, message: 'hello' })
            .expect(400);
    ...
    it('sends a chat message and returns reply', async () => {
        const a = await readyApp();
        vi.mocked(getCharacterById).mockResolvedValue({ id: 1, worldId: 1, name: 'Alice' });
...
describe('GET /api/chat/character/:id/history', () => {
    it('resolves latest session when sessionId omitted and returns history', async () => {
        const a = await readyApp();
        vi.mocked(getCharacterById).mockResolvedValue({ id: 1, worldId: 1, name: 'Alice' });
        vi.mocked(getLatestSessionForCharacter).mockResolvedValue({ id: 7, characterId: 1, worldId: 1, userId: null, title: null, summary: null, createdAt: new Date(), updatedAt: new Date() });
        const history = [
    ...
    it('returns empty history when no session exists and sessionId omitted', async () => {
        const a = await readyApp();
        vi.mocked(getCharacterById).mockResolvedValue({ id: 1, worldId: 1, name: 'Alice' });
        vi.mocked(getLatestSessionForCharacter).mockResolvedValue(null);
        const res = await request(a.server)
            .get('/api/chat/character/1/history')
            .expect(200);
...
describe('GET /api/chat/character/:id/sessions', () => {
    it('returns sessions for a character', async () => {
        ...
        vi.mocked(listCharacterChatSessions).mockResolvedValue(sessions);
        const res = await request(a.server)
...
describe('GET /api/relationships', () => {
    it('lists relationships', async () => {
        const a = await readyApp();
        vi.mocked(listRelationships).mockResolvedValue([{ id: 1, fromCharacterId: 1, toCharacterId: 2 }]);
        const res = await request(a.server)
...
describe('GET /api/relationships/character/:characterId', () => {
    it('returns relationships for a character', async () => {
        const a = await readyApp();
        vi.mocked(getCharacterById).mockResolvedValue({ id: 1, worldId: 1, name: 'Alice' });
        vi.mocked(getRelationshipsForCharacter).mockResolvedValue([{ id: 1, fromCharacterId: 1, toCharacterId: 2 }]);
        const res = await request(a.server)
...
describe('GET /api/settings/provider', () => {
    it('returns provider config', async () => {
        const a = await readyApp();
        vi.mocked(resolveProviderConfig).mockReturnValue({ provider: 'openai', chatModel: 'gpt-4o-mini' });
        const res = await request(a.server)
...
describe('GET /api/settings/image-provider', () => {
    it('returns image provider config', async () => {
        const a = await readyApp();
        vi.mocked(resolveImageProviderConfig).mockReturnValue({ provider: 'openai-image', enabled: true });
        const res = await request(a.server)
