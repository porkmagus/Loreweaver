...
async function main() {
    ...
    try {
        ...
        if (code !== 0)
            throw new Error(`Typecheck failed with exit code ${code}`);
        // 2. Unit tests
        console.log('\n🧪 Running unit tests...');
        code = await runCommand('npm', ['run', 'test']);
        if (code !== 0)
            throw new Error(`Unit tests failed with exit code ${code}`);
        ...
        if (code !== 0)
            throw new Error(`E2E tests failed with exit code ${code}`);
